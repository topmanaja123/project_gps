...
<body>
  ...
  if (isset($_POST['serach'])) {
    $sqlProsi="SELECT * FROM `positions` WHERE posi_id AND device_id ='7' AND devicetime BETWEEN '$_POST[date_start]' AND '$_POST[date_start]' order by devicetime DESC";
    $resultProsi = $conn->query($sqlProsi);
  ...
  <div class="form-row">
    <div class="col-md-3 table-responsive">
      ...
      <div class="container">
        <div class="form-row">
          <div class="col-sm-12 text-center">
            <button id='DLtoExcel' class="btn btn-info btn-sm" disabled>
      ...
      <div class="scrollbar">
        <!-- table show data -->
        <table id="tableData" class="table table-bordered table-sm">
          <thead>
            <tr id="history_head ">
          ...
          <tbody>
            ...
            if ($resultPosition) {
              while ($rs1 = $resultPosition->fetch_assoc()) {
                ?>
                <tr>
                  <td class="text-center" width="40%"><?= $rs1['devicetime']; ?></td>
...
<script>
  ...
  if ($resultPositionLine) {
    while ($resultPolyline = $resultPositionLine->fetch_assoc()) {
      ...
      marker = L.marker([<?= $resultPolyline['lat'] ?>, <?= $resultPolyline['lng'] ?>], {
        title: "<?= $s; ?>",
        icon: greenIcon,
...
?>
  ...
  var showLine = [latlng]; //push latlng in showline
